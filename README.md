# Groo Issues

This is the public issue tracker for all Groo products. Use this repository to report bugs, request features, or ask questions.

## Products

| Product | Description | Website |
|---------|-------------|---------|
| **Pass** | Password manager (web app + browser extension) | [pass.groo.dev](https://pass.groo.dev) |
| **Accounts** | Authentication service | [accounts.groo.dev](https://accounts.groo.dev) |
| **CLI** | Command-line tools | - |
| **Docs** | Documentation | [docs.groo.dev](https://docs.groo.dev) |
| **Drive** | File storage | [drive.groo.dev](https://drive.groo.dev) |
| **AI** | AI tools | [groo.dev](https://groo.dev) |
| **cl-wrangler** | Cloudflare account switcher | [GitHub](https://github.com/groo-dev/cl-wrangler) |

## How to Submit an Issue

1. Click the **Issues** tab above
2. Click **New Issue**
3. Choose a template:
   - **Bug Report** - Something isn't working
   - **Feature Request** - Suggest an improvement
4. Fill out the form and submit

## Security Vulnerabilities

**Do not report security vulnerabilities publicly.**

Please email [security@groo.dev](mailto:security@groo.dev) with details about the vulnerability. We will respond within 48 hours and work with you to address the issue.

## Support

For general support questions:
- Check the [documentation](https://docs.groo.dev)
- Email [support@groo.dev](mailto:support@groo.dev)
